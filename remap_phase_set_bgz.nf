...
process remap_phase_set_bgz {
  ...
  output:
    set sample, file("${sample}.remapped.vcf.bgz") into remappedVcfs

  """
  dsh-bio remap-phase-set -i $vcf -o ${sample}.remapped.vcf.bgz
  """
}

remappedVcfs.subscribe {
  println "Remapped ${it.get(0)} phase set into file ${it.get(1)}"
}
